 ...
 * 		var $name = 'Apple';
 * 		var $actsAs = array('Log' => array('userModel' => 'User', 'userKey' => 'user_id'));
 *  [..]
 * 
 ...
 * 
 * @author Marcos Trotti marcotrotti(at)gmail.com
 * @modified 03 Feb 2015 by Marcos Trotti
 * Compatible with cake 2.6.6
 * @version 0.5
 * 
 *  
 * Based On Work 
 * @authorAlexander Morland (alexander#maritimecolours.no)
...
class LogBehavior extends ModelBehavior {
	...
	function customLog(&$Model, $action, $id, $values = array()) {		
		...
		if (($this->Log->hasField('model_id')) && is_numeric($id)) {
    ...
    function _saveLog(&$Model, $logData, $title = null) {
    	...
    	if ( ($this->Log->hasField('model_id')) && !isset($logData['Log']['model_id'])) {
    		if ($Model->id) {
